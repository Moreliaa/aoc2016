input_a = ["abba[mnop]qrst", "abcd[bddb]xyyx", "aaaa[qwer]tyui", "ioxxoj[asdfgh]zxcvbn"]
input_b = ["aba[bab]xyz", "xyx[xyx]xyx", "aaa[kek]eke", "zazbz[bzb]cdb" ]
...
def check_substr(substr):
    for c in substr:
        if c == "[" or c == "]":
            return False
    return substr[3] == substr[0] and substr[2] == substr[1] and substr[2] != substr[0]
def check_substr_pt2(substr):
     for c in substr:
        if c == "[" or c == "]":
            return False
     return substr[2] == substr[0] and substr[1] != substr[0]
def check_against_ABA(substr, ABA):
    for aba in ABA:
        if substr[0] == aba[1] and substr[2] == aba[1] and substr[1] == aba[0]:
            return True
    return False
count = 0
count_pt2 = 0
for i in input:
    ...
    for idx in range(len(i) - 2):
        ...
        if c == "[":
            ...
            if idx + 4 <= len(i):
                result = check_substr(i[idx:idx+4])
                if result:
                    if match_pairs:
                        is_match = True
                    else:
                        invalid = True
            result_pt2 = check_substr_pt2(i[idx:idx+3])
            if result_pt2 and match_pairs:
                ABA.append(i[idx:idx+3])
    ...
    for idx in range(len(i) - 2):
        ...
        if c == "[":
            ...
            match_pairs = True
        elif not match_pairs:
            result_pt2 = check_against_ABA(i[idx:idx+3], ABA)
            if result_pt2: 
                is_match_pt2 = True
                break

    if is_match and not invalid:
        count +=1
    if is_match_pt2:
        print("OK: "+i)
        count_pt2 +=1
