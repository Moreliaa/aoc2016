...
for i in input:
    ...
    for c in name:
        if c == "-":
            continue
        if c in chars_in_name:
            chars_in_name[c] += 1
        else:
            chars_in_name[c] = 1
    sorted_chars = sorted(chars_in_name.items(), key=lambda chara: chara[1], reverse=True)
    most_common_counts = set(map(lambda chara: chara[1], sorted_chars[0:5]))
    most_common = sorted(list(map(lambda chara: (-chara[1], chara[0]), filter(lambda chara: chara[1] in most_common_counts, sorted_chars))))
    most_common = list(map(lambda chara: chara[1], most_common))
    isok = True
    for idx in range(len(chk)):
        c = chk[idx]
        if idx >= len(most_common) or c != most_common[idx]: 
           isok = False
           break
